...
function TaskList({jobs,handleDelete,handleEdit,handleUpdateEdit,handleStatus,handleDeleteAll,handleDeleteForever,handleRestore}) {
    ...
    <div className="col-12">
            ...
            <div className="table-responsive-xl">
                <table className="table-primary table-bordered  text-white" >
                    <thead className="text-center">
                        <tr>
                            <td className="col-1">Order</td>
                            <td className="col-5">Work list</td>
                            <td className="col-2">Status</td>
                            <td className="col-4" colSpan={3}>Option</td>
                        </tr>
                    </thead>
                    <tbody>
                        {jobs.filter(Filter_Map[filter]).filter((jobs)=>(!jobs.isDelete)).map((job,index)=>(
                                <tr key={index} style={job.status?{opacity:0.8}:{}}>
                                    ...
                                    <td className="col-1 text-center">
                                        {job.status?<div style={{color:'wheat',fontWeight:"bold"}}>Complete</div>:"Pending"}
                                    </td>
                                    <td className="col-1">
            ...
            <div className="col-lg-12 col-sm-12 mt-5">
                    <button type="button" className="btn btn-success" data-bs-toggle="modal" data-bs-target="#BinModal">
                        Trash Can
                        <span className="badge bg-primary" style={{marginLeft:"10px" }}>{lengthBin()===0?"":lengthBin()}</span>
                    </button>
                    <button type="button" className="btn btn-danger" data-bs-toggle="modal" data-bs-target="#exampleModal" style={{margin:"0px 10px"}}>
                        Delete All
                    </button>
                </div>
